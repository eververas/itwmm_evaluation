...
lms_indexes = mio.import_pickle('lsfm_lms_indexes.pkl')
...
#             corresponding images/ground truth meshes filenames + the suffix of
#             the mesh type (e.g .obj, .ply)
...
#  errors   : python list that contains the error per vertex for all meshes
    ...
    for i, filename in enumerate(print_progress(filenames)):
        ...
        errors[i] , _, _ = mask_align_and_calculate_dense_error(fit_3d, gt_mesh, eval_mask)
...
#             truth mesh and a mean mesh calculated from your reconstructed meshes
    ...
    mean_fit_3d.landmarks['ibug49'] = PointCloud(mean_fit_3d.points[lms_indexes][19:]) 

    # calculate per vertex errors between the neutral gt_mesh and the mean_fit_3d
    gt_mesh, eval_mask = landmark_and_mask_gt_mesh(gt_mesh, distance=1)
    errors[0] , _, _ = mask_align_and_calculate_dense_error(mean_fit_3d, gt_mesh, eval_mask)
    
    return errors
    
    
def plot_ced_3dMDLab(errors, method_name):
    
    errs = [error_vtx for error_mesh in errors for error_vtx in error_mesh]
    legs = [method_name]
    
    tab = statistics_table(errs, legs, 0.105, 0.01, precision=4, sort_by='auc')
    
    r =  plot_cumulative_error_distribution(errs, legend_font_size=20,
                                            line_width=5, marker_size=10, axes_font_size=20,
...
def plot_ced_4DMaja_synthetic(errors, method_name):
    return plot_ced_3dMDLab(errors, method_name)
...
def plot_ced_4DMaja_real(errors, method_name):
    return plot_ced_3dMDLab(errors, method_name)